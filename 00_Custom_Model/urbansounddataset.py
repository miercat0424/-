...
class UrbanSoundDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        signal = self.transformation(signal).to(self.device)               # -> mel_spectrogram passing the signal
        return signal, label
    ...
    def _get_audio_sample_label(self, index):               # -> take a label information
        # print("ANNOTATIONS : ",self.annotations.iloc[index, 6])
        return self.annotations.iloc[index, 6]

    # what item is useful // a_list[1] -> a_list.__getitem__(1)

if __name__ == "__main__":
    ANNOTATIONS_FILE = "D:\PyAud\xxx\Test2.csv"
    AUDIO_DIR = "D:\PyAud\xxx\WAV_Files" 
    SAMPLE_RATE = 22050
